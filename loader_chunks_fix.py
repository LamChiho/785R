...
class DataLoader:
    ...
    def __init__(self, data_file, rev_data_file=None, rev_id=None,  max_word=128, max_phrase=36, hold=False):
        ...
        # Load revision data only if files exist
        if rev_data_file and rev_id:
            if os.path.exists(rev_data_file) and os.path.exists(rev_id):
                try:
                    print(f"Loading revision data from {rev_data_file}...")
                    rev_data = load_pkl(rev_data_file)
                    rev_ids = load_pkl(rev_id)
                    original_size = len(self.dataset)
                    for rid in rev_ids:
                        sample = rev_data[rid]
                        sample['y'] = 3  # Label as special "revision" class
                        self.dataset.append(sample)
                    print(f"✓ Added {len(rev_ids)} revision samples ({original_size} → {len(self.dataset)} total)")
                except Exception as e:
                    print(f"⚠️  Error loading revision data: {e}")
                    print(f"   Continuing without revision samples...")
            else:
                print(f"⚠️  Revision files not found (will be generated during training)")
                if rev_data_file and not os.path.exists(rev_data_file):
                    print(f"   Missing: {rev_data_file}")
                if rev_id and not os.path.exists(rev_id):
                    print(f"   Missing: {rev_id}")
    
        assert self.max_word == len(self.dataset[0]['token_idxs'])
        self.sample_index = np.arange(0, len(self.dataset))
        print(f'Training Samples: {len(self.dataset)} Loaded')
        self.pos = 0
